...
public class HommingMissile_Tamaplayer : MonoBehaviour
{
    public float speed;
    ...
    private void Start()
    {
        ...
        targets = GameObject.FindGameObjectsWithTag("EnemyMinion");
        target = serchTag(gameObject, "EnemyMinion");
        target2 = serchTag(gameObject, "Turret");
    ...
    {
        if (collision.gameObject.CompareTag("EnemyMinion"))//さっきつけたTagutukeruというタグがあるオブジェクト限定で～という条件の下
        {
